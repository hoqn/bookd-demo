...
export default function ScrapCard({ data, className, onDeleteButtonClick, onImageGenButtonClick, ...restProps }: ScrapCardProps) {
  return <div className={twMerge("card relative", className)} {...restProps}>
    ...
    <div className="p-4 border-t border-t-slate-300 flex flex-row justify-end space-x-2">
      <Button tint="neutral" className="" onClick={onDeleteButtonClick}>삭제</Button>
      <Button tint="primary" intent="contained" className="" onClick={onImageGenButtonClick}>이미지 생성</Button>
    </div>
  </div>
}